...
describe("generateViteConfig", () => {
  it("returns valid vite config with vinuxt plugin", () => {
    ...
    expect(config).toContain('import vinuxt from "@raytien/vinuxt"');
...
describe("addScripts", () => {
  it("adds dev:vinuxt, build:vinuxt, start:vinuxt scripts", () => {
    ...
    const pkg = JSON.parse(
      fs.readFileSync(path.join(tmp_dir, "package.json"), "utf-8"),
    );
...
describe("getInitDeps", () => {
  it("returns vinuxt and vite", () => {
    ...
    expect(deps).toContain("@raytien/vinuxt");
...
describe("isDepInstalled", () => {
  ...
  it("returns true when dep is in devDependencies", () => {
    fs.writeFileSync(
      path.join(tmp_dir, "package.json"),
      JSON.stringify({ devDependencies: { "@raytien/vinuxt": "^0.1.0" } }),
    );
    expect(isDepInstalled(tmp_dir, "@raytien/vinuxt")).toBe(true);
  });
  ...
  it("returns false when dep is not installed", () => {
    ...
    expect(isDepInstalled(tmp_dir, "@raytien/vinuxt")).toBe(false);
  });
...
describe("init", () => {
  it("generates vite.config.ts", async () => {
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "test-nuxt-app",
        devDependencies: { "@raytien/vinuxt": "^0.1.0", vite: "^7.0.0" },
      }),
    );
    fs.writeFileSync(
    ...
    const config = fs.readFileSync(
      path.join(tmp_dir, "vite.config.ts"),
      "utf-8",
    );

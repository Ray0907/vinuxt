...
export interface InitOptions {
	 ...
	 */
	_exec?: (cmd: string, args: string[], opts: { cwd: string; stdio: string }) => void;
}
...
function installDeps(
	root: string,
	deps: string[],
	exec: (cmd: string, args: string[], opts: { cwd: string; stdio: string }) => void,
): void {
...
export async function init(options: InitOptions): Promise<InitResult> {
	...
	const exec = options._exec ?? ((cmd: string, args: string[], opts: { cwd: string; stdio: string }) => {
		execFileSync(cmd, args, opts as Parameters<typeof execFileSync>[1]);
	});
	...
	if (!hasNuxtConfig(root)) {
		...
		console.log("  Proceeding anyway -- vinuxt can work without a Nuxt config.\n");
	}
	...
	if (!skipped_vite_config) {
		...
		fs.writeFileSync(path.join(root, "vite.config.ts"), config_content, "utf-8");
	...
	if (deps_missing.length > 0) {
		console.log(`    \u2713 Added ${deps_missing.join(", ")} to devDependencies`);
	}

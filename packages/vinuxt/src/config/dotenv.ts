...
export type VinuxtEnvMode = "development" | "production" | "test";
 ...
 */
export function getDotenvFiles(mode: VinuxtEnvMode): string[] {
	return [
		`.env.${mode}.local`,
		...(mode === "test" ? [] : [".env.local"]),
		`.env.${mode}`,
 ...
 *
 * - **vinuxt** populates `process.env` so that server-side code (SSR, API
 *   routes, Server Components) can read env vars at runtime, and so the Vite
 *   plugin's `config()` hook can scan `process.env` for `NUXT_PUBLIC_*` vars
 *   to inline via `define`.
 *
 * - **Vite** loads .env files to populate `import.meta.env.VITE_*` for its
 *   own client exposure mechanism (which Nuxt apps don't use).
 *

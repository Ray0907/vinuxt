...
export function parseDeployArgs(args: string[]) {
  ...
  return {
    ...
    tprCoverage: values["tpr-coverage"]
      ? parseInt(values["tpr-coverage"], 10)
      : undefined,
    tprLimit: values["tpr-limit"]
      ? parseInt(values["tpr-limit"], 10)
      : undefined,
    tprWindow: values["tpr-window"]
      ? parseInt(values["tpr-window"], 10)
      : undefined,
  };
}
 ...
 */
export function isPackageResolvable(
  root: string,
  package_name: string,
): boolean {
  try {
...
function installDeps(root: string, deps: MissingDep[]): void {
  ...
  const args =
    pm === "npm" ? ["install", "-D", ...dep_args] : ["add", "-D", ...dep_args];
